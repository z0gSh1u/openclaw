...
async function resolveNativeHookRelayInvocationBinding(
  ...
  if (nativeAgentId && event === "pre_tool_use" && lifetime.retained && lifetime.retention) {
    ...
    const assertActive = () => {
      ...
      if (!retention.allowPreToolUse(rawPayload)) {
        throw new Error("native hook relay retained invocation not allowed");
      }
    };
    return {
